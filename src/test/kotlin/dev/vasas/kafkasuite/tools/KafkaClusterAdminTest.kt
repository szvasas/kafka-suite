...
class KafkaClusterAdminTest : KafkaSuite {
    ...
    inner class `when a new topic is created` {
        ...
        fun `the topic is available in the cluster with correct parameters`() {
            val createdTopic = kafkaCluster.listTopics()
                    .find {
                        it.name() == topicName
                    } ?: throw NoSuchElementException("Test topic is not created!")
        ...
        inner class `when the previously created topic is deleted` {
            ...
            @Test
            fun `listTopics() does not return the topic`() {
                val testTopic = kafkaCluster.listTopics().find {
                    it.name() == topicName
                }
                assertThat(testTopic).isNull()
            }
        }
